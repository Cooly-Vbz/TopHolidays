...
export default function AccountOverlay({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  const renderProfileTab = () => {
        ...
        <div>
          ...
          <form onSubmit={handleAuthSubmit} style={{ display: 'grid', gap: 8, maxWidth: 360 }}>
            <label style={{ display: 'grid', gap: 4 }}>
              <span>Email</span>
              <input type="email" required value={email} onChange={e => setEmail(e.target.value)} style={{ padding: 8, borderRadius: 6, border: '1px solid #D1D5DB' }} />
            </label>
            <label style={{ display: 'grid', gap: 4 }}>
              <span>Password</span>
              <input type="password" required minLength={6} value={password} onChange={e => setPassword(e.target.value)} style={{ padding: 8, borderRadius: 6, border: '1px solid #D1D5DB' }} />
            </label>
            {mode === 'signup' && (
              <label style={{ display: 'grid', gap: 4 }}>
                <span>Display name (encrypted)</span>
                <input value={displayName} onChange={e => setDisplayName(e.target.value)} style={{ padding: 8, borderRadius: 6, border: '1px solid #D1D5DB' }} />
              </label>
            )}
            {error && <div style={{ color: '#B91C1C', fontSize: 13 }}>{error}</div>}
            {message && <div style={{ color: '#047857', fontSize: 13 }}>{message}</div>}
            <button type="submit" style={{ marginTop: 4, padding: '8px 12px', borderRadius: 8, border: 'none', background: '#10B981', color: '#fff' }}>
              {mode === 'signin' ? 'Sign in' : 'Create account'}
            </button>
            <button type="button" onClick={() => setMode(mode === 'signin' ? 'signup' : 'signin')} style={{ border: 'none', background: 'transparent', color: '#2563EB', fontSize: 13, textAlign: 'left', padding: 0 }}>
    ...
    return (
      <form onSubmit={handleProfileSave} style={{ display: 'grid', gap: 8, maxWidth: 420 }}>
        <div style={{ marginBottom: 8 }}>
          <div style={{ fontWeight: 600 }}>Profile</div>
          <div style={{ fontSize: 13, color: '#6B7280' }}>Update your display name. It is stored encrypted locally.</div>
        </div>
        ...
        <div style={{ display: 'flex', gap: 8, marginTop: 4 }}>
          ...
          <button type="button" onClick={signOut} style={{ padding: '8px 12px', borderRadius: 8, border: '1px solid #DC2626', background: '#FFF5F5', color: '#B91C1C' }}>Sign out</button>
        </div>
      </form>
    )
  }
  ...
  const renderSecurityTab = () => (
    <div style={{ display: 'grid', gap: 12, maxWidth: 480 }}>
      <div>
        ...
        <div style={{ fontSize: 13, color: '#6B7280' }}>Toggle basic security preferences for this device.</div>
      </div>
  ...
  const renderPaymentTab = () => (
    <div style={{ display: 'grid', gap: 12, maxWidth: 520 }}>
      <div>
        ...
        <div style={{ fontSize: 13, color: '#6B7280' }}>Stored locally for demo purposes only.</div>
      </div>
      {settings.paymentMethods.length === 0 ? (
        <div style={{ fontSize: 14, color: '#6B7280' }}>No payment methods saved.</div>
      ) : (
        <ul style={{ listStyle: 'none', padding: 0, margin: 0, display: 'grid', gap: 8 }}>
          {settings.paymentMethods.map(pm => (
            <li key={pm.id} style={{ padding: 8, borderRadius: 8, border: '1px solid #E5E7EB', display: 'flex', justifyContent: 'space-between' }}>
              ...
              <button
                ...
                style={{ border: 'none', background: 'transparent', color: '#B91C1C', fontSize: 13 }}
              >Remove</button>
            </li>
          ))}
        </ul>
      ...
      <button
        ...
        style={{ alignSelf: 'flex-start', padding: '8px 12px', borderRadius: 8, border: '1px dashed #9CA3AF', background: '#F9FAFB' }}
  ...
  const renderAddressesTab = () => (
    <div style={{ display: 'grid', gap: 12, maxWidth: 520 }}>
      <div>
        ...
        <div style={{ fontSize: 13, color: '#6B7280' }}>Shipping addresses stored only in this browser.</div>
      </div>
      {settings.addresses.length === 0 ? (
        <div style={{ fontSize: 14, color: '#6B7280' }}>No addresses saved.</div>
      ) : (
        <ul style={{ listStyle: 'none', padding: 0, margin: 0, display: 'grid', gap: 8 }}>
          {settings.addresses.map(addr => (
            <li key={addr.id} style={{ padding: 8, borderRadius: 8, border: '1px solid #E5E7EB' }}>
              <div style={{ fontWeight: 600 }}>{addr.label}</div>
              <div style={{ fontSize: 14 }}>{addr.line1}</div>
              <div style={{ fontSize: 13, color: '#6B7280' }}>{addr.city}, {addr.country}</div>
              <button
                type="button"
                onClick={() => updateSettings(prev => ({
                  ...prev,
                ...
                style={{ marginTop: 4, border: 'none', background: 'transparent', color: '#B91C1C', fontSize: 13 }}
              >Remove</button>
            </li>
          ))}
        </ul>
  ...
  const renderPrivacyTab = () => (
    <div style={{ display: 'grid', gap: 12, maxWidth: 520 }}>
      <div>
        ...
        <div style={{ fontSize: 13, color: '#6B7280' }}>Control how this app uses your data on this device.</div>
      </div>
      <label style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
        <input
          type="checkbox"
      ...
      <button
        ...
        style={{ marginTop: 8, padding: '8px 12px', borderRadius: 8, border: '1px solid #DC2626', background: '#FEF2F2', color: '#B91C1C' }}
  ...
  return (
    <div role="dialog" aria-modal style={{ position: 'fixed', inset: 0, zIndex: 1100 }}>
      ...
      <div style={{ position: 'absolute', top: '10%', left: '10%', right: '10%', bottom: '10%', background: '#fff', borderRadius: 12, boxShadow: '0 8px 24px rgba(0,0,0,0.2)', overflow: 'auto', animation: 'fadeIn 300ms', maxHeight: '80vh' }}>
        ...
        <header style={{ padding: 12, borderBottom: '1px solid #E5E7EB', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          ...
          <button onClick={onClose} aria-label="Close" style={{ border: 'none', background: 'transparent', fontSize: 18 }}>⬇️</button>
        </header>
            ...
            {tabLabels.map(label => (
              <button
                ...
                style={{
                  ...
                  background: tab === label ? '#EFF6FF' : 'transparent',
                  color: tab === label ? '#1D4ED8' : '#111827',
                }}
              >{label}</button>
            ))}
          </aside>
